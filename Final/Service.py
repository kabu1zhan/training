...
def death_effect (engine, hero):
    engine.hero = Objects.Death(hero)
    engine.notify("You are immortality!")
...
class EmptyMap(MapFactory):
    ...
    class Map:
        def __init__(self):
            self.Map = [[0 for _ in range(15)] for _ in range(15)]
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['objects']:
                ...
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(1, 13), random.randint(1, 13))
                    intersect = True
                    while intersect:
                        intersect = False
        ...
        def get_objects(self, _map):
            ...
            for obj_name in self.config:
                ...
                for i in range(self.config[obj_name]):
                    coord = (random.randint(1, 11), random.randint(1, 8))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
