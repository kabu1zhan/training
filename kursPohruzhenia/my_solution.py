'''Путеводитель'''
.capitalize() - делает заглавную букву строки большой
.isdigit() - проверяет является ли строка числом
in проверяет подстроку в строке
f"" - форматировать строку
byte строки = b'' кодировать с помощью .encode(encoding = 'кодировка')
.decode() = разкодировать
pass - не делает ничего, игнорирует
