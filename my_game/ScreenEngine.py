...
class GameSurface(ScreenHandle):
    ...
    def draw_map(self):
        ...
        min_x = self.game_engine.hero.position[0] - self.game_engine.map_left_top[0]
        min_y = self.game_engine.hero.position[1] - self.game_engine.map_left_top[1]
...
class MiniMap(ScreenHandle):
    ...
    def draw(self, canvas):
        if self.game_engine.show_mini_map:

            if self.game_engine.level in (5, 6):
                mini_map = [[0 for _ in range(39)] for _ in range(11)]
            else:
                mini_map = [[0 for _ in range(41)] for _ in range(41)]

            for i in range(len(mini_map[0])):
                for j in range(len(mini_map)):
                    if self.game_engine.map[j][i] == Service.wall:
                        mini_map[j][i] = Service.map_wall
                    elif self.game_engine.map[j][i] == Service.floor1:
                        mini_map[j][i] = Service.map_floor1
                    elif self.game_engine.map[j][i] == Service.floor2:
                        mini_map[j][i] = Service.map_floor2
                    elif self.game_engine.map[j][i] == Service.floor3:
                        mini_map[j][i] = Service.map_floor3

            for i in range(len(mini_map[0])):
                for j in range(len(mini_map)):
                    canvas.blit(mini_map[j][i][0],
                                (i * self.game_engine.map_sprite_size, j * self.game_engine.map_sprite_size))

            canvas.blit(Service.map_hero[0], (self.game_engine.hero.position[0] * self.game_engine.map_sprite_size,
                                              self.game_engine.hero.position[1] * self.game_engine.map_sprite_size))

            for obj in self.game_engine.objects:
                if type(obj) is Objects.Ally:
                    canvas.blit(Service.map_obj[0], ((obj.position[0]) * self.game_engine.map_sprite_size,
                                                     (obj.position[1]) * self.game_engine.map_sprite_size))
                if type(obj) is Objects.Enemy:
                    canvas.blit(Service.map_enemy[0], ((obj.position[0]) * self.game_engine.map_sprite_size,
                                                       (obj.position[1]) * self.game_engine.map_sprite_size))
