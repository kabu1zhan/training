...
class GameSurface(ScreenHandle):
    ...
    def calculate(self):
        ...
        [x, y] = self.game_engine.hero.position
        x *= size
        y *= size

        map_size_x = len(self.game_engine.map[0]) * size
        map_size_y = len(self.game_engine.map) * size

        [display_size_x, display_size_y] = self.get_size()
        if display_size_x < map_size_x:
            if x > display_size_x / 2 and (map_size_x - x) >= display_size_x / 2:
                min_x += int((x - display_size_x / 2) / size)
            elif (map_size_x - x) < display_size_x / 2:
                min_x = int((map_size_x - display_size_x) / size)
            else:
                min_x = 0

        if display_size_y < map_size_y:
            if y > display_size_y / 2 and (map_size_y - y) >= display_size_y / 2:
                min_y += int((y - display_size_y / 2) / size)
            elif (map_size_y - y) < display_size_y / 2:
                min_y = int((map_size_y - display_size_y) / size)
            else:
                min_y = 0

        return min_x, min_y

    def calculate_size_mini(self):

        map_size_x = len(self.game_engine.map[0])
        map_size_y = len(self.game_engine.map)

        [display_size_x, display_size_y] = self.get_size()
        return min(int(display_size_x / map_size_x), int(display_size_y / map_size_y))
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        self.blit(font.render('Hero at {}'.format(self.engine.hero.position), True, colors["black"]),
                  (250, 0))
        ...
        self.blit(font.render('{0}/{1}'.format(self.engine.hero.hp, self.engine.hero.max_hp), True, colors["black"]),
                  (60, 30))
        self.blit(font.render('{0}/{1}'.format(self.engine.hero.exp, (100*(2**(self.engine.hero.level-1)))), True, colors["black"]),
                  (60, 70))
        ...
        self.blit(font.render('{}'.format(self.engine.hero.stats["strength"]), True, colors["black"]),
                  (480, 30))
...
class HelpWindow(ScreenHandle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.data.append([" Esc ", "Close game"])
